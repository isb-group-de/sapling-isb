...
export function useSaplingNote() {
  ...
  watch(() => currentPersonStore.person?.handle, loadNotesForGroup)

  watch(() => i18n.global.locale.value, reloadAll)
  // #endregion
  ...
  async function reloadAll() {
    await Promise.all([currentPersonStore.fetchCurrentPerson(), loadGroups()])
