...
occurrence without a duplicate. Before opening the occurrence draft, the editor
checks the selected start against the freshly loaded series. If a stale calendar
entry was already detached or is no longer generated by the current rule, the
calendar refreshes instead of submitting an invalid second detachment.
