...
| `backend/src/api/generic/generic.service.ts` | Still coordinates a broad set of generic CRUD workflows after update-conflict evaluation, change-log persistence, and open-task notifications moved into dedicated services. | Extract import orchestration or timeline record loading when those paths are touched again. |
...
| `frontend/src/components/dialog/SaplingDialogEdit.vue` | Dialog shell remains medium-sized after header/status chips, form section rendering, auxiliary dialogs, and record-action orchestration moved into child components/composables. | Extract relation-tab orchestration or reference-template field syncing when those paths are touched again. |
