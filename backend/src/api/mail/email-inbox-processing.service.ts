import { currentPromptManifest } from '../ai/prompts/ai-prompt-context';
...
export class EmailInboxProcessingService {
  ...
  async processInboundEmail(handle: number): Promise<void> {
    ...
    return new AiPromptService(em).run(async () => {
      email.promptManifest = currentPromptManifest();
