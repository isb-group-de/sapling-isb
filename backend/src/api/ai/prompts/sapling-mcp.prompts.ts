export const SAPLING_MCP_TOOL_DESCRIPTIONS = {
  ...
  genericCreate:
    'Create a Sapling generic record with the same insert permissions as the current user. Inspect required fields and reference fields with entity_schema before creating an unfamiliar entity. For every reference field, use referenceName to look up the target record with generic_list and submit its referencedPks value (usually handle), never its display label. Treat autogenerated handles or primary keys in the result as internal metadata and do not expose them in normal user-facing prose unless the user explicitly asks for them.',
  genericUpdate:
...
export const SAPLING_MCP_USAGE_HINTS = {
  ...
  entitySchema: [
    ...
    'For a reference field, referenceName identifies the target entity and referencedPks/referencePrimaryKeys identify the required key values. Look up the intended target record with generic_list and write those key values; never write displayValue or another display label into the reference field.',
  ...
  toolError: [
    ...
    'Reference fields require primary-key values from referencedPks. Look up the target entity from referenceName with generic_list; never retry with a display label.',
