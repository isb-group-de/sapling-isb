...
export class SystemTelemetryController {
  ...
  async recordClientMetric(@Body() dto: ClientMetricTelemetryDto) {
    if (!this.acceptWithinLimit())
      return { accepted: false, rateLimited: true };
    return { accepted: await this.persistClientMetrics([dto]) };
  }

  @Post('api/system/telemetry/client-metrics')
  @HttpCode(HttpStatus.ACCEPTED)
  async recordClientMetrics(@Body() dto: ClientMetricTelemetryBatchDto) {
    if (!this.acceptWithinLimit())
      return { accepted: false, rateLimited: true };
    return { accepted: await this.persistClientMetrics(dto.metrics) };
  }

  private async persistClientMetrics(
    ...
    const rows = (await em.getConnection().execute(
      `with active_instance as (
        ...
        select * from jsonb_to_recordset(cast(? as jsonb)) as metric(
          "metricKey" varchar, "value" float8, "page" varchar
        )
      )
      ...
      ) select active_instance."handle",
        date_bin(interval '1 minute', now(), timestamp '2000-01-01'), '1m',
        metrics."metricKey", metrics."page", 1, metrics."value", metrics."value",
        metrics."value", metrics."value", now()
      from active_instance cross join metrics
      ...
      returning "handle"`,
      [this.environment.currentId, JSON.stringify(metrics)],
